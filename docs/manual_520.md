...
You can use the following command to pull the specific version of toolchain 520, for example, v1.0.0 which is described
in this document.

```bash
docker pull kneron/toolchain:520_v1.0.0
```
...
`kneron/toolchain:520_v1.0.0` as the docker image. Before we actually start the docker, we'd better provide a folder
...
```bash
docker run --rm -it -v /mnt/docker:/docker_mount kneron/toolchain:520_v1.0.0
```
...
Under `/data1/compiler`, we can find three binary files generated by compiler which are the inputs of the simulator.
