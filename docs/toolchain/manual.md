...
* `id` is the identifier of the model. It should be a integer greater than 0. ID before 1000 are reserved for Kneron models. Please use ID greater than 1000 for custom models.
...
For this example, we load create the LittleNet ModelConfig with the following python code:
...
```python
hw_results = ktc.kneron_inference(input_data, nef_file=compile_result, model_id=1001, radix=ktc.get_radix(input_images))
```
