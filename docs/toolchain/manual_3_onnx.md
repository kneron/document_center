...
* datapath_bitwidth_mode: choose from "int8"/"int16"/"mix balance"/"mix light". ("int16" is not supported in kdp520. "mix balance" and "mix light" are combines of int8 and int16 mode. "mix balance" prefers int16 while "mix light" prefers int8. If the model has ReduceL2 operator, we suggest using "mix light" or "int16".)
...
* compiler_tiling: tiling optimization. Could be "default" or "deep_search". Deep search mode optimizes the performance but takes longer. Defaults to "default".
